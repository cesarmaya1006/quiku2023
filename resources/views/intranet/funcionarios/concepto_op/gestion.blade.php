    ...
    <div class="row justify-content-center">
        <div class="col-12 col-md-11 d-flex align-items-stretch flex-column">
            <div class="card card-primary">
                ...
                <div class="card-body">
                    <form action="{{ route('funcionario-gestionar_conceptoUOpinion_guardar') }}" method="POST" autocomplete="off"
                        ...
                        <div class="col-12 rounded border mb-3 p-2">
                            <div class="row">
                                ...
                                <div class="col-12 col-md-6">
                                    Tipo Solicitud: <strong>{{ $concepto->tipoPqr->tipo }}</strong>
                                </div>
                                @if ($concepto->adquisicion == 'Sede física')
                                <div class="col-12 col-md-6">
                                    Departatmento : <strong>{{ $concepto->sede->municipio->departamento->departamento
                                        }}</strong>
                                </div>
                                <div class="col-12 col-md-6">
                                    Municipio : <strong>{{ $concepto->sede->municipio->municipio }}</strong>
                                </div>
                                <div class="col-12 col-md-6">
                                    Sede : <strong>{{ $concepto->sede->nombre }}</strong>
                                </div>
                                @endif
                                <div class="col-12 col-md-6">
                                    Plazo de respuesta prórroga días hábiles: <strong>{{ $concepto->prorroga_dias }}</strong>
                                </div>
                                <div class="col-12 col-md-6">
                                    Fecha de radicado: <strong>{{ $concepto->fecha_radicado }}</strong>
                                </div>
                                <div class="col-12 col-md-6">
                                    Fecha estimada de respuesta:
                                    <strong>{{ date('Y-m-d', strtotime($concepto->fecha_generacion . '+ ' .
                                        ($concepto->tiempo_limite) . ' days')) }}</strong>
                                </div>
                                <div class="col-12 col-md-6">
                                    Estado: <strong>{{ $concepto->estado->estado_funcionario }}</strong>
                                </div>
                            </div>

                            @foreach ($concepto->peticiones as $solicitud)
                                @php
                                        ...
                                        <div class="row form-respuesta-recursos">
                                            ...
                                            @if ($variableRecursoApelacion && $recurso->tipo_reposicion_id == 3)
                                                <div class="card-footer d-flex justify-content-end guardarRespuestaRecurso">
                                                    <button type="" class="btn btn-primary px-4" data_url="{{ route('respuesta_recurso_guardar_cuo') }}" data_url_anexos="{{ route('respuesta_recurso_anexos_guardar_cuo') }}" data_token="{{ csrf_token() }}">Guardar recurso</button>
                                                </div>
                                            @elseif($recurso->tipo_reposicion_id == 1 || $recurso->tipo_reposicion_id == 2)     
                                                <div class="card-footer d-flex justify-content-end guardarRespuestaRecurso">
                                                    <button type="" class="btn btn-primary px-4" data_url="{{ route('respuesta_recurso_guardar_cuo') }}" data_url_anexos="{{ route('respuesta_recurso_anexos_guardar_cuo') }}" data_token="{{ csrf_token() }}">Guardar recurso</button>
                                ...
                                <select class="form-control form-control-sm col-12 col-md-6" name="prioridad" required>
                                    @foreach ($estadoPrioridad as $prioridad)
                                        <option value="{{ $prioridad->id }}"
                                            {{ $concepto->prioridad_id == $prioridad->id ? 'selected' : '' }}>
                                            {{ $prioridad->prioridad }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <hr>
                            @if($concepto->estadospqr_id < 6 && $concepto->prorroga == 0 || $concepto->estadospqr_id > 6 && $concepto->prorroga == 1)
                                <div class="row pb-3 form-respuestaProrroga">
                                    <input class="respuestaProrroga" type="hidden" value="{{$concepto->prorroga}}">
                                    <div class="col-12 col-md-6 ">
                                        <h6>Prórroga</h6>
                                    </div>
                                    <div class="col-12 col-md-6 d-flex flex-row">
                                        ...
                                        <div class="col-12 d-flex row">
                                            <div class="col-12 form-group">
                                                ...
                                                <input type="number" class="form-control form-control-sm plazo_prorroga col-md-3"
                                                    ...
                                                    max="{{$concepto->tipoPqr->tiempos * 2}}">
                                                <p>El máximo de días de prórroga es: {{$concepto->tipoPqr->tiempos * 2}}</p>
                                            </div>
                                            <div class="col-12 d-flex row">
                                                <label for="prorroga_pdf">Justificacion de prórroga</label>
                                                <textarea type="text" class="form-control form-control-sm prorroga_pdf"
                                                    name="prorroga_pdf" id="prorroga_pdf">{{$concepto->prorroga_pdf}}</textarea>
                                            </div>
                                        </div>
                                        @if($concepto->estadospqr_id < 6 && $concepto->prorroga == 0)
                                            <div class="card-footer d-flex justify-content-end" id="guardarProrroga">
                                                <button type="" class="btn btn-primary px-4" data_url="{{ route('prorroga_guardar_cuo') }}" data_token="{{ csrf_token() }}">Guardar prórroga</button>
                                            </div>
                                        @endif
                                    </div>
                                </div>
                        ...
                        <div class="card-footer d-flex justify-content-end">
                            ...
                            @if(!($concepto->estadospqr_id == 6 || $concepto->estadospqr_id == 10))
                        ...
                        <input class="totalPeticiones" id="totalPeticiones" name="totalPeticiones" type="hidden"
                            value="{{$concepto->id}}">
                        <input class="id_pqr" id="id_pqr" name="id_pqr" type="hidden" value="{{$concepto->id}}">
                        <input class="id_estado_pqr" id="id_estado_pqr" type="hidden" value="{{$concepto->estadospqr_id}}">
                    </form>
                </div>
            </div>
        </div>
...
@section('scripts_pagina')
    <script src="{{ asset('js/intranet/conceptouopinion/gestion.js') }}"></script>
@endsection
<!-- ************************************************************* -->