            ...
            <div class="card card-primary">
                <div class="card-header">
                    <h3 class="card-title">Gestión a Solicitud Número de radicado:
                        <strong>{{ $denuncia->radicado }}</strong>
                    </h3>
                </div>
                <div class="card-body">
                    <form action="{{ route('funcionario-gestionar_denuncia_guardar') }}" method="POST" autocomplete="off"
                        ...
                        <div class="col-12 rounded border mb-3 p-2">
                            <div class="row">
                                <div class="col-12 col-md-6">
                                    ...
                                    <strong>{{ $denuncia->persona->nombre1 . ' ' . $denuncia->persona->nombre2 . ' ' .
                                        $denuncia->persona->apellido1 . ' ' . $denuncia->persona->apellido2 }}</strong>
                                    @else
                                    Empresa que interpone la Solicitud:
                                    <strong>{{ $denuncia->empresa->razon_social . ' ' . $denuncia->empresa->razon_social . ' ' .
                                        $denuncia->empresa->razon_social . ' ' . $denuncia->empresa->razon_social }}</strong>
                                    @endif
                                </div>
                                <div class="col-12 col-md-6">
                                    Tipo Solicitud: <strong>{{ $denuncia->tipoPqr->tipo }}</strong>
                                </div>
                                @if ($denuncia->adquisicion == 'Sede física')
                                <div class="col-12 col-md-6">
                                    Departatmento : <strong>{{ $denuncia->sede->municipio->departamento->departamento
                                        }}</strong>
                                </div>
                                <div class="col-12 col-md-6">
                                    Municipio : <strong>{{ $denuncia->sede->municipio->municipio }}</strong>
                                </div>
                                <div class="col-12 col-md-6">
                                    Sede : <strong>{{ $denuncia->sede->nombre }}</strong>
                                </div>
                                @endif
                                <div class="col-12 col-md-6">
                                ...
                                <div class="col-12 col-md-6">
                                    ...
                                    <strong>{{ date('Y-m-d', strtotime($denuncia->fecha_generacion . '+ ' .
                                        ($denuncia->tiempo_limite) . ' days')) }}</strong>
                                </div>
                                <div class="col-12 col-md-6">
                                    Estado: <strong>{{ $denuncia->estado->estado_funcionario }}</strong>
                                </div>
                            </div>

                            @foreach ($denuncia->irregularidades as $solicitud)
                                @php
                                                ...
                                                <div class="col-12">
                                                    ...
                                                    @if (!$solicitud->otro)
                                                        <div class="row mt-2">
                                                            ...
                                                            <div class="col-12">{{ $solicitud->irregularidad }}</div>
                                                        </div>
                                                    @else
                                                    <div class="row mt-2">
                                                        <h6>Tipo de irregularidad:</h6>
                                                        <div class="col-12">
                                                            <p class="text-justify"><strong>{{ $solicitud->irregularidad }}:</strong> {{ $solicitud->otro }}</p>
                                                        </div>
                                                    </div>
                                                    @endif
                                                </div>
                                        ...
                                        <div class="row form-respuesta-recursos">
                                            ...
                                            @if ($variableRecursoApelacion && $recurso->tipo_reposicion_id == 3)
                                                <div class="card-footer d-flex justify-content-end guardarRespuestaRecurso">
                                                    <button type="" class="btn btn-primary px-4" data_url="{{ route('respuesta_recurso_guardar_d') }}" data_url_anexos="{{ route('respuesta_recurso_anexos_guardar_d') }}" data_token="{{ csrf_token() }}">Guardar recurso</button>
                                                </div>
                                            @elseif($recurso->tipo_reposicion_id == 1 || $recurso->tipo_reposicion_id == 2)     
                                                <div class="card-footer d-flex justify-content-end guardarRespuestaRecurso">
                                                    <button type="" class="btn btn-primary px-4" data_url="{{ route('respuesta_recurso_guardar_d') }}" data_url_anexos="{{ route('respuesta_recurso_anexos_guardar_d') }}" data_token="{{ csrf_token() }}">Guardar recurso</button>
                                ...
                                <select class="form-control form-control-sm col-12 col-md-6" name="prioridad" required>
                                    @foreach ($estadoPrioridad as $prioridad)
                                        <option value="{{ $prioridad->id }}"
                                            {{ $denuncia->prioridad_id == $prioridad->id ? 'selected' : '' }}>
                                            {{ $prioridad->prioridad }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <hr>
                            @if($denuncia->estadospqr_id < 6 && $denuncia->prorroga == 0 || $denuncia->estadospqr_id > 6 && $denuncia->prorroga == 1)
                                    ...
                                    <div class="col-12 contentProrroga" id="contentProrroga">
                                        <div class="col-12 d-flex row">
                                            <div class="col-12 form-group">
                                                ...
                                                <p>El máximo de días de prórroga es: {{$denuncia->tipoPqr->tiempos * 2}}</p>
                                            </div>
                                            <div class="col-12 d-flex row">
                        ...
                        <div class="card-footer d-flex justify-content-end">
                            ...
                            @if(!($denuncia->estadospqr_id == 6 || $denuncia->estadospqr_id == 10))
                        ...
                        <input class="totalPeticiones" id="totalPeticiones" name="totalPeticiones" type="hidden"
                            value="{{$denuncia->id}}">
                        <input class="id_pqr" id="id_pqr" name="id_pqr" type="hidden" value="{{$denuncia->id}}">
                        <input class="id_estado_pqr" id="id_estado_pqr" type="hidden" value="{{$denuncia->estadospqr_id}}">
                    </form>
                </div>
            </div>
        </div>
...
@section('scripts_pagina')
    <script src="{{ asset('js/intranet/denuncia/gestion.js') }}"></script>
@endsection
<!-- ************************************************************* -->