            ...
            <div class="card card-primary">
                <div class="card-header">
                    <h3 class="card-title">Gestión a Solicitud Número de radicado:
                        <strong>{{ $solicitudDatos->radicado }}</strong>
                    </h3>
                </div>
                <div class="card-body">
                    <form action="{{ route('funcionario-gestionar_solicitudDatos_guardar') }}" method="POST" autocomplete="off"
                        ...
                        <div class="col-12 rounded border mb-3 p-2">
                            <div class="row">
                                <div class="col-12 col-md-6">
                                    @if ($solicitudDatos->persona_id != null)
                                    Persona que interpone la Solicitud:
                                    <strong>{{ $solicitudDatos->persona->nombre1 . ' ' . $solicitudDatos->persona->nombre2 . ' ' .
                                        $solicitudDatos->persona->apellido1 . ' ' . $solicitudDatos->persona->apellido2 }}</strong>
                                    @else
                                    Empresa que interpone la Solicitud:
                                    <strong>{{ $solicitudDatos->empresa->razon_social . ' ' . $solicitudDatos->empresa->razon_social . ' ' .
                                        $solicitudDatos->empresa->razon_social . ' ' . $solicitudDatos->empresa->razon_social }}</strong>
                                    @endif
                                </div>
                                <div class="col-12 col-md-6">
                                    Tipo Solicitud: <strong>{{ $solicitudDatos->tipoPqr->tipo }}</strong>
                                </div>
                                @if ($solicitudDatos->adquisicion == 'Sede física')
                                <div class="col-12 col-md-6">
                                    Departatmento : <strong>{{ $solicitudDatos->sede->municipio->departamento->departamento
                                        }}</strong>
                                </div>
                                <div class="col-12 col-md-6">
                                    Municipio : <strong>{{ $solicitudDatos->sede->municipio->municipio }}</strong>
                                </div>
                                <div class="col-12 col-md-6">
                                    Sede : <strong>{{ $solicitudDatos->sede->nombre }}</strong>
                                </div>
                                @endif
                                <div class="col-12 col-md-6">
                                    Plazo de respuesta prórroga días hábiles: <strong>{{ $solicitudDatos->prorroga_dias }}</strong>
                                </div>
                                <div class="col-12 col-md-6">
                                    Fecha de radicado: <strong>{{ $solicitudDatos->fecha_radicado }}</strong>
                                </div>
                                <div class="col-12 col-md-6">
                                    Fecha estimada de respuesta:
                                    <strong>{{ date('Y-m-d', strtotime($solicitudDatos->fecha_generacion . '+ ' .
                                        ($solicitudDatos->tiempo_limite) . ' days')) }}</strong>
                                </div>
                                <div class="col-12 col-md-6">
                                    Estado: <strong>{{ $solicitudDatos->estado->estado_funcionario }}</strong>
                                </div>
                            </div>
                        ...
                        <div class="col-12 peticion_general rounded border mb-3 p-2">
                                        ...
                                        <div class="col-12">
                                            <div class="row">
                                                <div class="col-12">
                                                    <div class="row">
                                                        <div class="col-12">
                                                            <h5>Solicitud #{{$numerador}}</h5>
                                                        </div>
                                                        <div class="col-12">{{ $solicitud->tiposolicitud }}</div>
                                                    </div>
                                                    <div class="row mt-2">
                                                        <h6>Datos personales objeto de la solicitud:</h6>
                                                        <div class="col-12 text-justify">{{ $solicitud->datossolicitud }}</div>
                                                    </div>
                                                    <div class="row mt-2">
                                                        <h6>Descripción de la solicitud:</h6>
                                                        <div class="col-12 text-justify">{{ $solicitud->descripcionsolicitud }}</div>
                                                    </div>
                                                </div>
                            ...
                            <div class="row">
                                ...
                                @if($solicitudDatos->estadospqr_id < 6)
                            ...
                            <div class="row">
                                ...
                                <div class="row">
                                    ...
                                    @php
                                        ...
                                        if(sizeOf($solicitud->recursos) > 2){
                                            if($solicitud->recursos[1]->respuestaRecurso){
                                        ...
                                        <div class="row form-respuesta-recursos">
                                            ...
                                            @if ($variableRecursoApelacion && $recurso->tipo_reposicion_id == 3)
                                                <div class="card-footer d-flex justify-content-end guardarRespuestaRecurso">
                                                    <button type="" class="btn btn-primary px-4" data_url="{{ route('respuesta_recurso_guardar_sd') }}" data_url_anexos="{{ route('respuesta_recurso_anexos_guardar_sd') }}" data_token="{{ csrf_token() }}">Guardar recurso</button>
                                                </div>
                                            @elseif($recurso->tipo_reposicion_id == 1 || $recurso->tipo_reposicion_id == 2)
                                                <div class="card-footer d-flex justify-content-end guardarRespuestaRecurso">
                                                    <button type="" class="btn btn-primary px-4" data_url="{{ route('respuesta_recurso_guardar_sd') }}" data_url_anexos="{{ route('respuesta_recurso_anexos_guardar_sd') }}" data_token="{{ csrf_token() }}">Guardar recurso</button>
                                ...
                                <select class="form-control form-control-sm col-12 col-md-6" name="prioridad" required>
                                    @foreach ($estadoPrioridad as $prioridad)
                                        <option value="{{ $prioridad->id }}"
                                            {{ $solicitudDatos->prioridad_id == $prioridad->id ? 'selected' : '' }}>
                                            {{ $prioridad->prioridad }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <hr>
                            @if($solicitudDatos->estadospqr_id < 6 && $solicitudDatos->prorroga == 0 || $solicitudDatos->estadospqr_id > 6 && $solicitudDatos->prorroga == 1)
                                <div class="row pb-3 form-respuestaProrroga">
                                    <input class="respuestaProrroga" type="hidden" value="{{$solicitudDatos->prorroga}}">
                                    <div class="col-12 col-md-6 ">
                                        <h6>Prórroga</h6>
                                    </div>
                                    <div class="col-12 col-md-6 d-flex flex-row">
                                        ...
                                        <div class="col-12 d-flex row">
                                            <div class="col-12 form-group">
                                                ...
                                                <input type="number" class="form-control form-control-sm plazo_prorroga col-md-3"
                                                    ...
                                                    max="{{$solicitudDatos->tipoPqr->tiempos * 2}}">
                                                <p>El máximo de días de prórroga es: {{$solicitudDatos->tipoPqr->tiempos * 2}}</p>
                                            </div>
                                            <div class="col-12 d-flex row">
                                                <label for="prorroga_pdf">Justificacion de prórroga</label>
                                                <textarea type="text" class="form-control form-control-sm prorroga_pdf"
                                                    name="prorroga_pdf" id="prorroga_pdf">{{$solicitudDatos->prorroga_pdf}}</textarea>
                                            </div>
                                        </div>
                                        @if($solicitudDatos->estadospqr_id < 6 && $solicitudDatos->prorroga == 0)
                                            <div class="card-footer d-flex justify-content-end" id="guardarProrroga">
                                                <button type="" class="btn btn-primary px-4" data_url="{{ route('prorroga_guardar_sd') }}" data_token="{{ csrf_token() }}">Guardar prórroga</button>
                                            </div>
                                        @endif
                                    </div>
                                </div>
                                        ...
                                        @if ($plazoRecurso == 0)
                                            <div class="col-12 col-md-3 form-group">
                                                ...
                                                <input type="number" class="form-control form-control-sm plazo_recurso"
                                                    ...
                                                    max="{{$solicitudDatos->tipoPqr->tiempos}}">
                                            </div>
                                        @else
                        ...
                        <div class="card-footer d-flex justify-content-end">
                            ...
                            @if(!($solicitudDatos->estadospqr_id == 6 || $solicitudDatos->estadospqr_id == 10))
                        ...
                        <input class="totalPeticiones" id="totalPeticiones" name="totalPeticiones" type="hidden"
                            value="{{$solicitudDatos->id}}">
                        <input class="id_pqr" id="id_pqr" name="id_pqr" type="hidden" value="{{$solicitudDatos->id}}">
                        <input class="id_estado_pqr" id="id_estado_pqr" type="hidden" value="{{$solicitudDatos->estadospqr_id}}">
                    </form>
                </div>
            </div>
        </div>
...
@section('scripts_pagina')
    <script src="{{ asset('js/intranet/solicituddatos/gestion.js') }}"></script>
@endsection
<!-- ************************************************************* -->